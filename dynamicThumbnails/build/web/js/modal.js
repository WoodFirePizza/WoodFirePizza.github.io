...
$('body').on('click', 'a.modal', function() {
            ...
            modalFooter += '<div class="btn-group btn-group-justified">';
                            ...
                            modalFooter += '<a  class="basket btn btn-success btn-lg" data-src="Res/Images/Pizza/bbqchicken.jpg" data-name="' + name + '" data-price="' + price + '" data-description="delicious" data-key="'+ key +'" data-dismiss="modal">Add To Basket</a>';
            modalFooter += '</div>';
            
            $('#myModal').find('.modal-title').text(name);
            $('#myModal').find('.modal-body').html(modalBody);
            $('#myModal').find('.modal-footer').html(modalFooter);
            $('#myModal').modal('show');
        });
