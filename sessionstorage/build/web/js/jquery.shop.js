	...
	$.Shop.prototype = {
		...
		displayCart: function() {
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					$tableCartBody.html( "<div class='alert alert-danger' role='alert' id='idEmptyCartMessage'>Your basket is empty</div>" );	
				} else {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						var html = "<tr>'<td class='pimg text-center'><img width='30px' height='30px' src='" + image + "'/></td>" +
                                                        "<td class='pname'>" + product + "</td>" + "<td class='psize'>" + size + "</td>" +"<td class='pqty'><input type='number' min='1' style='width: 70px;' value='" + qty + "' class='qty' id='update-cart'/></td>"; 
					    	html += "<td class='pprice'>" + price + "</td><td class='pdelete text-center'><a href='' class='btn btn-xs pull-left'   data-product='" + product + "'><i class='fa fa-times-circle-o' aria-hidden='true'></i></a></td></tr>";
				...
				var basketItems = checkoutCart.items;
				var $basketBody = this.$checkoutCart.find( "tbody" );

				if( basketItems.length > 0 ) {
				
					for( var j = 0; j < basketItems.length; ++j ) {
						var summaryItem = basketItems[j];
						var summaryProduct = summaryItem.product;
						var summaryPrice = this.currency + " " + summaryItem.price;
						var summaryQty = summaryItem.qty;
                                                var summarySize = summaryItem.size;
                                                var summaryImage = summaryItem.image;
						var summaryHTML = "<tr>'<td class='pimg'><img width='30px' height='30px' src='" + summaryImage + "'/></td>" + "<td class='pname'>" + summaryProduct + "</td>" + "<td class='psize'>" + summarySize + "</td>" + "<td class='pqty'>" + summaryQty + "</td>" + "<td class='pprice'>" + summaryPrice + "</td></tr>";
					
						$basketBody.html( $basketBody.html() + summaryHTML );
                                                
					}
				} else {
					$basketBody.html( "<div class='alert alert-danger' role='alert' id='idEmptyCartMessage'>Your basket is empty</div>" );
			...
			self.$updateCartBtn.on( "click", function() {
				...
				$rows.each(function() {
					...
					var pname = $.trim( $row.find( ".pname" ).text() );
                                        var psize = $.trim( $row.find( ".psize" ).text() );
                                        var pimg = $.trim( $row.find( ".pimg" ).children('img').attr('src'));
			...
			self.$formAddToCart.each(function() {
				...
				$form.on( "submit", function() {
					var qty = self._convertString( $form.find( ".qty" ).val() );
                                        var size = self._convertNumber( $form.find( ".size" ).val() );
