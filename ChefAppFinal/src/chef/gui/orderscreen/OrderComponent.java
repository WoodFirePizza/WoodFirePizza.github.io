    ...
    public void paintComponent(Graphics g){
        ...
        if(!order.getPizzaItems().isEmpty()){
            for(Pizza currPizza: order.getPizzaItems()){
                ...
                g.setColor(Color.WHITE);
                g.drawString(currPizza.getQuantity() + "x " + currPizza.getName(), 5, y);
                y += LINE_SIZE;
                ...
                for(Ingredient currIng: currPizza.getIngredients()){
                    if(currIng.getIsExtra() || currIng.getIsRemoved()){
                        ...
                        g.drawString(currIng.showIngredientDetail(), 15, y);
                        y += LINE_SIZE;
                    }
                }
        ...
        if(!order.getSideItems().isEmpty()){
            ...
            for(Side currSide: order.getSideItems()){
                g.drawString(currSide.getQuantity() + "x " + currSide.getName(), 5, y);
                y += LINE_SIZE;
            }
        ...
        if(!order.getDessertItems().isEmpty()){
            ...
            for(Dessert currDessert: order.getDessertItems()){
                g.drawString(currDessert.getQuantity() + "x " + currDessert.getName(), 5, y);
                y += LINE_SIZE;
            }  
        ...
        g.drawString(String.valueOf("Total: £" + df.format(order.getOrderPrice())), 350, y);
